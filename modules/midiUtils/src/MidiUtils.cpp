...
void MidiMessageQueue::printfMicroLL( void )
{
  ...
  {
	  localPrintf("%d, %d, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%08X, 0x%02X\n",
				i,
				tempObject->tick,
				tempObject->controlMask,
				tempObject->channel,
				tempObject->value,
				tempObject->data,
				(uint32_t)tempObject,
				(uint32_t)tempObject->nextObject);
    //move note
...
void MidiMessageLinkedList::printDebug( void )
{
	...
	localPrintf("MidiMessageLinkedList listing\n");
